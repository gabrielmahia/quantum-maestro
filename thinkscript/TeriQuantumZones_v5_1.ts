# =====================================================================
# TeriQuantumZones v5.1 - IWT buyer/seller zones + market permission
#
# FIXES vs the v5.0 draft (all verified):
#  1. BASE COUNT >=5 no longer collapses to 0. A long tight base (Teri's
#     STRONGEST setup) previously scored baseCount=0 => zone discarded.
#     Now compact candles are counted and clamped to [1,4]; a 5+ base is
#     treated as a 4-candle base (strongest), never thrown away.
#  2. SCORE THRESHOLDS aligned to the canonical cohorts:
#     7-8 PRIMARY (direct at proximal) | 5-6 SECONDARY (confirmation, half)
#     0-4 SKIP. v5.0 used 6/4 which shifted every cohort down by one.
#  3. SECONDARY cohort is no longer dead. v5.0 hard-required RR>=3 in
#     longZoneQualified, which blocked every 2-3R secondary trade the
#     score system was designed to allow. Now: PRIMARY needs RR>=3;
#     SECONDARY needs RR>=2 AND confirmation. The score gate and the RR
#     gate agree instead of contradicting.
#  4. NON-REPAINTING zone lines. v5.0 used PaintingStrategy.HORIZONTAL,
#     which paints today's zone backward across all history - misleading
#     on scan-back. Lines now plot only on bars where the zone is active,
#     showing where zones actually were.
#  5. BEARISH permission clarified: bearish setups gate on the correct
#     rail - long puts (debit) allowed defensively; bearish CREDIT (short
#     premium) still requires shortPremiumAllowed (no vol stress / no ER).
#  6. TR-departure caution: default stays body (a long-wick rejection bar
#     should NOT count as a strong departure); label warns if TR mode on.
#
# Mechanical approximation of Teri Ijeoma's visual method. Cannot fully
# reproduce discretionary U/Chair reading. Defined-risk decision support
# only - selects no strikes, sends no orders. App engine remains canonical.
# =====================================================================
