# ═══════════════════════════════════════════════════════════════════
# TeriQuantumOsc v4.6 - definitive lower-panel study
#
# Lineage: v4.4 (regime/instrument split + earnings/ex-div/RS gates)
#          + v5 (on-chart eight-point ZONE ODDS enhancer)
#          + v4.6 canonical alignment (this version):
#   - Freshness matches the course odds table exactly: 0 visits=2,
#     1 visit=1, MORE THAN 1 ("1+ VISITS")=0.
#   - Canonical order-type intent shown on chart: ENTRY=limit,
...
#   LOCATION   <- CHART SYMBOL. This name's buyer/seller levels + room.
#   PREMIUM    <- CHART SYMBOL IV. Rich/cheap for THIS name's options.
#   PERMISSION <- events, vol stress, earnings, ex-div.
#
# Proxy only. Quantum Maestro app engine remains canonical. Zone
# detection is a mechanical approximation, NOT validated against Teri's
# hand-marked zones. No 0DTE, defined risk only, no short premium into
# events. Selects no strikes, sends no orders.
# ═══════════════════════════════════════════════════════════════════
...
# ── Canonical order-type + reward:risk gate (IWT BUY worksheet) ──────
# Entry & profit-exit are LIMIT (price control); the protective stop is
# STOP-MARKET (fill certainty when the level breaks). The worksheet rule
# is "RATIO 3+ TAKE THE TRADE"; 2-3R is secondary, <2R rejects.
AddLabel(enableOddsEnhancer and !IsNaN(depStrength),
    "ORDERS: entry LIMIT / exit LIMIT / stop STOP-MKT" +
    "  |  RR gate: " +
    (if rrProxy >= 3 then ">=3 TAKE" else if rrProxy >= 2 then "2-3 secondary" else "<2 REJECT"),
    if rrProxy >= 3 then Color.GREEN else if rrProxy >= 2 then Color.ORANGE else Color.GRAY);
